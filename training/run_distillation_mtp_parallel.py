...
from create_student_model_mtp import StudentModelMTPParallel, compute_parallel_mtp_loss
...
DatasetDict({
    ...
    config = WhisperConfig.from_pretrained(
        ...
        token=model_args.token,
    ) # model_args.model_name_or_path='./distil-large-v3-init-debug-mtp-en4-de2-parallel-n3', student model; NOTE 因为传入的是path，所以cache_dir没有用
    ...
    print(teacher_model) # 1,541,570,560 ipdb> sum(p.numel() for p in teacher_model.parameters() if p.requires_grad)
    ...
    print(student_model) # 754,485,760 (en32,de2) is with 48.9% of its teacher_model (half size). ||| 203,553,280 (en4,de2) is with 13.20% of its teacher_model
    ...
    if training_args.freeze_encoder: # True, NOTE if student_model.en layer < teacher_model.en layer, better set freeze_encoder=False
    ...
    share_hidden_states = training_args.freeze_encoder and student_model.config.d_model == teacher_model.config.d_model and student_model.config.encoder_layers == teacher_model.config.encoder_layers # TODO add an additional condition here 
    ...
    if training_args.do_train and not is_skip_wer_filtering:
    ...
    if training_args.do_eval and not is_skip_wer_filtering:
        for eval_split in all_eval_splits:
    ...
    def train_step(
        ...
        loss = 0.64 * ce_loss + 0.16 * mtp_ce_loss + training_args.kl_weight * kl_loss
